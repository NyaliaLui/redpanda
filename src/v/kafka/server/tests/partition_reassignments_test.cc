...
FIXTURE_TEST(
  test_list_partition_reassignments, partition_reassignments_test_fixture) {
    ...
    {
        test_log.info("List partition assignments {}", test_tp);
        ...
        BOOST_CHECK_EQUAL(resp.data.topics.size(), 1);
        for (auto& tp_resp : resp.data.topics) {
            BOOST_CHECK(tp_resp.name() == test_tp());
            BOOST_CHECK_EQUAL(tp_resp.partitions.size(), num_partitions);
            for (auto& p_resp : tp_resp.partitions) {
                BOOST_CHECK(
                  std::find(pids.begin(), pids.end(), p_resp.partition_index)
                  != pids.end());
                BOOST_CHECK_EQUAL(p_resp.replicas.size(), 1);
                BOOST_CHECK_EQUAL(p_resp.replicas[0], model::node_id{1});
                BOOST_CHECK(p_resp.adding_replicas.empty());
                BOOST_CHECK(p_resp.removing_replicas.empty());
            }
        }
    }

    {
        test_log.info("List all ongoing reassignments (expect empty)");
